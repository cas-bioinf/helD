...
if(!all(colnames(counts3)[2:length(counts3)] == samples3)) {
  stop("Mismatched count columns and sample info")
}
...
sampleInfo_all <- #rbind(sampleInfo1, sampleInfo2, sampleInfo3) %>%
  rbind(sampleInfo2, sampleInfo3) %>% 
...
res_helD %>% as.data.frame() %>% rownames_to_column("gene") %>% filter(padj <= 0.05)
